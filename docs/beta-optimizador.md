...
La web lee el catálogo desde `programs/{id}` y, después de comprobar el acceso, el HTML completo desde `programContents/{id}`. **Subir estos archivos a GitHub no actualiza automáticamente la calculadora almacenada en Firebase.** La migración de documentos está descrita en [`firebase-content-migration.md`](firebase-content-migration.md).
...
23 pruebas automatizadas aprobadas: 16 del optimizador, 6 de acceso y 1 de demo segura. Incluyen demandas justo por encima de un armado, todos los diámetros disponibles, coherencia de áreas y cantidades, costos cero, campos inválidos, rangos extremos, limpieza de resultados y la ausencia del HTML privado en la vista pública. Se ejecuta el script real contenido en el HTML; los tests no mantienen una segunda implementación del motor.
...
2. **Completar la migración del contenido de pago.** Los documentos antiguos pueden conservar un campo público `html`; hay que moverlo a `programContents/{id}`, eliminarlo de `programs/{id}` y publicar reglas que protejan esa colección. Bloquear campos en una demo no sustituye esta separación.
3. **Revisar aislamiento y aprobación de pagos.** Los iframes se cargan sin `allow-same-origin` y las solicitudes siguen requiriendo pruebas de las reglas y de la aprobación atómica. La separación impide la descarga por visitantes no autorizados, pero el código enviado a un usuario autorizado puede inspeccionarse en su navegador.
...
Para probar la fuente del optimizador localmente, servir la carpeta del proyecto y abrir `/programs/optimizador-vigas.html`. Los gráficos requieren conexión a sus CDN. La web principal consulta el catálogo actual de Firebase; la vista pública usa una demo segura y el programa completo se carga desde `programContents` después de la migración.
